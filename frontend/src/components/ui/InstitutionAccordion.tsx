  ...
  return (
    <li className="py-1 text-sm sm:flex sm:items-baseline sm:gap-2">
      <span className="flex min-w-0 items-baseline gap-2">
        <span
          aria-hidden="true"
          className="h-1.5 w-1.5 shrink-0 -translate-y-0.5 rounded-[2px] bg-orange"
        />
...
function InstitutionRow({ institution }: { institution: Institution }) {
  ...
  const panelId = useId();
  const total = coursesOf(institution).length;
                ...
                <div className="flex flex-wrap items-baseline gap-x-2 gap-y-1">
                  ...
                  <span
                    className={`shrink-0 rounded-full border-2 border-stroke-soft px-2 py-0.5 font-mono text-[10px] whitespace-nowrap ${
                      track.status === "Concluída" ? "bg-mint text-on-accent" : "bg-yellow text-on-accent"
                    }`}
                  >
                    {track.status}
                  </span>
